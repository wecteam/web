...
hexo.extend.helper.register('cate_menu', function(className){
  var menu = this.site.data.cates,
    ...
    self = this,
    lang = this.page.lang,
    isDefaultLang = lang === 'zh-cn' || lang === 'default',
    path1 = this.path,
    isActive = function(path0){
        if(path0.indexOf('index.html') != -1) {
            return path1 === path0;    
        }
        return (path1.indexOf(path0) !== -1);    
    }

  menu && menu.forEach(function(obj) {
      var path = obj.url;
...
hexo.extend.helper.register('page_keywords', function(asStr){
    ...
    var siteKeywords = hexo.config.keywords ? hexo.config.keywords.split(', ') : [];
